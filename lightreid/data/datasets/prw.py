...
class PRW(ReIDSamples):
    """PRW.
    Reference:
        Lianing Zheng. Person Re-identification in the Wild.
    ...
    """
    data_path = "http://zheng-lab.cecs.anu.edu.au/Project/project_prw.html"
    def __init__(self, data_path, combineall=False, download=False):

        # is not exist and download true, download dataset or stop
        if not os.path.exists(data_path):
            if download:
                print('dataset path {} is not existed, start download dataset at'.format(data_path))
        ...
        query_path = os.path.join(data_path, 'query_box/')
        ...
        # init
        super(PRW, self).__init__(train, query, gallery, combineall)
